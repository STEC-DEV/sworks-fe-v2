...
const FacilityEditForm = ({
  ...
  const form = useForm<FacilityFormType>({
    ...
    defaultValues: {
      elvPassenger: building?.elvPassenger ?? 0,
      elvCargo: building?.elvCargo ?? 0,
      elvEmr: building?.elvEmr ?? 0,
      subsCapacity: building?.subsCapacity ?? 0,
      genCapacity: building?.genCapacity ?? 0,
      upsYn: building?.upsYn ?? false,
          ...
          {subFields.length > 0 ? (
            <div className="flex flex-col gap-3">
              {subFields.map((d, i) => (
                <div
                  key={i + d.id}
                  className="grid grid-cols-1 xl:grid-cols-5 gap-4 p-4 rounded-xl bg-background border border-border items-end"
                >
                  {basicCode.hcCodes && (
                    <FormField
                      control={form.control}
            ...
            <EmptyBox message="냉난방 항목을 추가해주세요." />
          )}
        </FormCard>
